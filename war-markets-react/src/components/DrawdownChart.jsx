...
function CustomTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: t.border, border: "1px solid #475569", borderRadius: 8, padding: "12px 16px", fontSize: 13, color: t.textHigh, boxShadow: "0 8px 24px rgba(0,0,0,0.4)" }}>
...
export default function DrawdownChart() {
  ...
  return (
    <section style={card}>
      <h2 style={{ fontSize: 20, fontWeight: 600, color: t.textHigh, marginBottom: 4 }}>Maximum Drawdown by Conflict</h2>
      <p style={{ fontSize: 13, color: t.textMute, marginBottom: 20 }}>Peak-to-trough decline from the pre-war close</p>
      <ResponsiveContainer width="100%" height={Math.max(combinedData.length * 78, 460)}>
        <BarChart data={combinedData} layout="vertical" margin={{ top: 5, right: 50, left: 10, bottom: 5 }}>
          ...
          <Legend wrapperStyle={{ fontSize: 12, color: t.textMid, paddingTop: 8 }} iconType="square" iconSize={10} />
          <Bar dataKey="spDecline" name="S&P 500" fill={t.indigo} radius={[0, 5, 5, 0]} barSize={20} />
          <Bar dataKey="nqDecline" name="NASDAQ" fill={t.green} radius={[0, 5, 5, 0]} barSize={20} />
        </BarChart>
      </ResponsiveContainer>
    </section>
  );
