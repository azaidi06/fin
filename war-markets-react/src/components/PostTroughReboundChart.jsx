...
export default function PostTroughReboundChart() {
  ...
  return (
    <section style={card}>
      ...
      <p style={{ fontSize: 13, color: t.textMute, marginBottom: 4 }}>
        S&P 500 % return from trough close at each weekly interval. Hover a conflict to highlight its path.
      </p>
      {hasIran && (
        <p style={{ fontSize: 12, color: t.amberStrong, marginBottom: 16, lineHeight: 1.5 }}>
      ...
      <ResponsiveContainer width="100%" height={380}>
        <LineChart data={chartData} margin={{ top: 16, right: 30, left: 0, bottom: 8 }}>
          ...
          <XAxis
            dataKey="week"
            stroke={t.axis}
            tick={{ fill: t.textMute, fontSize: 12 }}
            tickFormatter={v => `W${v}`}
            type="number"
            domain={[2, 26]}
          ...
          <ReferenceLine y={0} stroke={t.textLow} strokeWidth={1} />
          {sortedKeys.map(k => {
            const isIran = k === "Iran";
            const isHovered = hovered === k;
            const dimmed = hovered != null && !isHovered && !isIran;
            const color = fiscalConflictColors[k] || t.textMute;
            return (
              <Line
                key={k}
                type="monotone"
          ...
          return (
            <button
              ...
              style={{
                ...
                color: hovered === k || isIran ? t.textHigh : t.textMid,
            ...
            >
              ...
              {k}
              <span style={{ color: t.textLow, fontWeight: 400 }}>
